...
namespace GoM
{
    class Program
    {
        ...
        {
            var app = new CommandLineApplication(throwOnUnexpectedArg: false) { Name = "GoM" };
            ...
            app.Command("add", (command) =>
            {
                ...
                command.OnExecute(() =>
                {
                    ...
                    if (repos > 0)
                    {
                        var projectPath = !string.IsNullOrEmpty(projectLocationArgument.Value) ? projectLocationArgument.Value : Directory.GetCurrentDirectory();
                    ...
                    {
                        var nameBranch = !string.IsNullOrEmpty(projectLocationArgument.Value) ? projectLocationArgument.Value : null;
                        try
                        {
            ...
            app.Command("remove", (command) =>
            {
                ...
                command.OnExecute(() =>
                {
                    ...
                    var path = !string.IsNullOrEmpty(projectLocationArgument.Value) ? projectLocationArgument.Value : Directory.GetCurrentDirectory();
                    var name = !string.IsNullOrEmpty(projectLocationArgument.Value) ? projectLocationArgument.Value : null;
                    ...
                    if (repo > 0)
                    {
                        try
                        {
                            ...
                            Console.WriteLine(removeRepo
                                ? "Repository successfully removed."
                                : "Repository wasn't removed.");
                        }
                        catch (Exception e)
                        {
            ...
            app.Command("fetch", c =>
            {
                ...
                c.OnExecute(() =>
                {
                    var path = !string.IsNullOrEmpty(repoPath.Value) ? repoPath.Value : Directory.GetCurrentDirectory();
                    var name = !string.IsNullOrEmpty(branchName.Value) ? branchName.Value : null;
                    ...
                    if (remoteBranches.Any(b => b.Name.Equals(name)))
                    {
            ...
            app.Command("files", c =>
            {
                ...
                c.OnExecute(() =>
                {
                    var projectPath = !string.IsNullOrEmpty(locationArgument.Value) ? locationArgument.Value : Directory.GetCurrentDirectory();
                    ...
                    else if (Directory.Exists(projectPath) && Repository.IsValid(projectPath))
                    {
                        ...
                        FileTree ft = new FileTree { Nodes = new List<FileTree>() };
